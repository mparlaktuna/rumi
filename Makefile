...
qemu: rumi initfs ## run linux kernel with qemu ctrl-a x exits from qemu
	qemu-system-x86_64 -kernel $(LINUX_BUILD)/arch/x86/boot/bzImage -nographic -append "earlyprintk=serial,ttyS0 console=ttyS0 debug" --initrd custom-initramfs.cpio.gz
